...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<Object> handleFeignBadRequest(FeignException.BadRequest ex) {
		logger.error("Error al consumir servicio externo: {}", ex.getMessage());

		// Extraer el cuerpo de la respuesta si está disponible
		String responseBody = ex.contentUTF8();
		String message = "Error en la solicitud al servicio externo.";
		if (responseBody != null && !responseBody.isEmpty()) {
			message = String.format("Error en la solicitud al servicio externo: %s", responseBody);
		}
